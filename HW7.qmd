...
```
Seeing the general trend, we can see that the outliers seem to be those observations which have studentized residuals beyond +-2. Again, we need to understand their influence on the model.
...
So from the result we can see that the observation point 24 seems to be having the highest influence by a large margin and hence can potentially affect the model's fit and predictions.

Now the Cook's distance values help us in identifying those influential points and hence we can observe that in the plot to ensure again that the regression model is not unduly influenced by a few observations and also improving the model's robustness and reliability.
